...
import 'package:newflutter/models/model_login.dart';
...
class LoginScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(create: (_) => LoginModel(),
    child: Scaffold(
        appBar: AppBar(),
        body: Column(
          children: [
            ...
            PasswordInput(),
            LoginButton(),
            Padding(padding: EdgeInsets.all(10),child: Divider(thickness: 1,),),
            RegisterButton(),
          ],
        )
    ),);
...
class EmailInput extends StatelessWidget {
  ...
  Widget build(BuildContext context){
    ...
    return Container(
      ...
      child: TextField(
        onChanged: (email) {login.setEmail(email);},
...
class PasswordInput extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    ...
    return Container(
      padding: EdgeInsets.all(10),
      child: TextField(
        onChanged: (password) {login.setPassword(password);},
        obscureText: true,
        decoration: InputDecoration(
            labelText: 'password',
            helperText: '',
